...
Cart is stored client-side in Level One (React Context + localStorage). The backend only validates cart contents during checkout. No persisted cart endpoints.
...
| POST | `/api/v1/payments/verify` | 🔓* | `{ authority, status }` | `{ success, orderId, refId }` | *Called by gateway callback; server verifies with IP + signature |
| GET | `/api/v1/payments/:authority` | 🔒 | — | `{ payment }` | Lookup payment status |

### 4.7 Admin Dashboard Helpers

| Method | Path | Auth | Response DTO | Notes |
|---|---|---|---|---|
...
- [ ] Passwords hashed with `argon2id` (never MD5/SHA1).
...
- [ ] No sensitive data inside JWT payload except `id`, `email`, `phone`, `role`.
- [ ] CORS restricted to known frontend origins.

### 9.2 Payment

- [ ] Verify every payment server-to-server; never trust callback query params alone.
- [ ] Amount in verify request matches stored payment amount.
- [ ] `authority`/`trackId` unique and tied to a single order.
- [ ] Gateway credentials (`merchant_id`, `API keys`) in environment variables only.
- [ ] Callback endpoint validates gateway IP / signature where supported.
- [ ] Stock decrement wrapped in the same transaction as payment success.
- [ ] No direct client-to-gateway verification; backend mediates all verification.

### 9.3 Admin Endpoints

- [ ] All `/api/v1/admin/*` routes require `ADMIN` role.
- [ ] Admin actions logged with actor `userId`, timestamp, and changed fields.
- [ ] Product/category mutations validated against slug uniqueness.
- [ ] Dashboard summary endpoint is rate-limited to prevent heavy queries.
- [ ] No mass-assignment vulnerabilities; DTOs explicitly whitelist fields.

### 9.4 Input & Output

- [ ] All inputs validated with Zod before reaching services.
- [ ] SQL injection prevented via ORM parameterized queries.
- [ ] XSS prevented by not returning unsanitized HTML; frontend uses safe rendering.
...
3. Generate shared TypeScript types for frontend consumption.
4. Begin `Jolfa-Server` setup with Express + Zod + Prisma + argon2.
