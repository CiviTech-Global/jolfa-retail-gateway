...
- `Price` — formatted Toman price with Persian numerals.
...
| **Catalog** | `/products` | `ProductGrid`, `ProductCard`, `FilterSidebar`, `SortSelect`, `Pagination` |
| **Product Detail** | `/products/:slug` | `ProductGallery`, `ProductInfo`, `QuantityStepper`, `AddToCartButton`, `RelatedProducts` |
...
| **Profile** | `/profile` | `ProfileForm`, `OrderHistory`, `OrderStatusBadge` |
| **Static** | `/about`, `/contact`, `/rules` | `StaticPageLayout`, `ContactForm` |
...
| Search | Debounce Persian input; normalize Arabic/Persian characters if needed later. |
...
- `@fontsource/vazirmatn` is optional if self-hosting font files; listed here for DX.
- Add `@tanstack/react-query-devtools` in dev if desired.
...
10. **Week 3, Day 19** — QA, responsive pass, RTL polish.
...
- All routes render without errors in RTL layout.
- Cart persistence works across refresh.
